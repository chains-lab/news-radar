...
func TestDislikesImpl_CreateAndGet(t *testing.T) {
	...
	dislikes, err := neodb.NewDislikes("neo4j://localhost:7687", "neo4j", "password")
	if err != nil {
		t.Fatalf("failed to create DislikesImpl: %v", err)
	}

	if err := dislikes.Create(ctx, userID, articleID); err != nil {
		t.Fatalf("failed to create DISLIKED relationship: %v", err)
	}

	articles, err := dislikes.GetForUser(ctx, userID)
	if err != nil {
		t.Fatalf("failed to get dislikes for user: %v", err)
	}
	found := false
	...
	if !found {
		t.Errorf("expected articleID %v in dislikes for user", articleID)
	}

	users, err := dislikes.GetForArticle(ctx, articleID)
	if err != nil {
		t.Fatalf("failed to get dislikes for article: %v", err)
	}
	found = false
	for _, u := range users {
		if u == userID {
	...
	if !found {
		t.Errorf("expected userID %v in dislikes for article", userID)
	}

	if err := dislikes.Delete(ctx, userID, articleID); err != nil {
		t.Fatalf("failed to delete DISLIKED relationship: %v", err)
	}

	articles, err = dislikes.GetForUser(ctx, userID)
	if err != nil {
		t.Fatalf("failed to get dislikes for user after delete: %v", err)
	}
	for _, a := range articles {
		if a == articleID {
			t.Errorf("expected DISLIKED relationship to be deleted, but found articleID %v", articleID)
		}
	}
}
